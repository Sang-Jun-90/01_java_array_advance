package step3_01.arrayAdvance;

import java.util.Arrays;
import java.util.Random;
import java.util.Scanner;

// # 소코반

public class ArrayEx43_정답 {

	public static void main(String[] args) {
		
		Scanner scan = new Scanner(System.in);
		Random ran = new Random();
		
		final int SIZE = 7;
		final int PLAYER = 2;
		final int BALL = 3;
		final int GOAL = 7;
		final int WALL = 9;
		
		int[][] map = new int[SIZE][SIZE];
		
		int wallCount = 0;
		int pY = 0;
		int pX = 0;
		int ballY = 0;
		int ballX = 0;
		int goalY = 0;
		int goalX = 0;
		
		// 벽설치
		System.out.print("설치할 벽의 개수를 입력하세요 : ");
		wallCount = scan.nextInt();
		
		int temp = wallCount;
		
		while (temp != 0) {
			int rY = ran.nextInt(SIZE);
			int rX = ran.nextInt(SIZE);
			
//			if (map[rY-1][rX-1] == BALL || map[rY-1][rX] == BALL || map[rY-1][rX+1] == BALL ||
//			map[rY][rX-1] == BALL ||                            map[rY][rX+1] == BALL ||
//			map[rY+1][rX-1] == BALL || map[rY+1][rX] == BALL || map[rY+1][rX+1] == BALL) {
//			continue;
//		}
			
			if (map[rY][rX] == 0) {
				map[rY][rX] = WALL;
				temp--;
			}			
		}
		
		// 공 설치
		while(true) {
			int rY = ran.nextInt(SIZE-2)+1;
			int rX = ran.nextInt(SIZE-2)+1;	
			int cnt = 0;
			for (int i = -1; i < 2; i++) {
				for (int j = -1; j < 2; j++) {
					if(map[rY+i][rX+j] == WALL) {
						cnt++;
					}
				}
			}
			
			if (map[rY][rX] == 0 && cnt < 2) {
				map[rY][rX] = BALL;
				ballY = rY;
				ballX = rX;
				break;
			}
		}
		
		// 골대 설치
		while(true) {
			int rY = ran.nextInt(SIZE);
			int rX = ran.nextInt(SIZE);
			if (map[rY][rX] == 0) {
				map[rY][rX] = GOAL;
				goalY = rY;
				goalX = rX;
				break;
			}
		}
		
		// 화면 출력
		for (int i = 0; i < SIZE; i++) {System.out.println(Arrays.toString(map[i]));}
		
		// 플레이어배치
		while(true ) {
			System.out.println(">>>플레이어의 위치를 선택하세요.");
			System.out.print("y좌표 입력 : ");pY = scan.nextInt();
			System.out.print("x좌표 입력 : ");pX = scan.nextInt();
			if(map[pY][pX]==0) {map[pY][pX]=PLAYER;break;}
		}
		
		// 게임시작
		while (true) {
			// 화면출력
			System.out.println("---------------");
			for (int i=0; i<SIZE; i++) {
				for (int j=0; j<SIZE; j++) {
					if (map[i][j] == PLAYER) 	System.out.print("* ");
					else if(map[i][j] == WALL) 	System.out.print("| ");
					else if(map[i][j] == GOAL) 	System.out.print("◎ ");
					else if(map[i][j] == BALL) 	System.out.print("● ");
					else 						System.out.print(map[i][j] + " ");
				}
				System.out.println();
			}
			System.out.println("---------------");
			// 게임 종료
			if (ballY == goalY && ballX == goalX) {System.out.println("게임종료");break;}
			
			System.out.print("상(1)하(2)좌(3)우(4) 입력 : ");
			int move = scan.nextInt();
			
			// 현재위치 저장
			
			int yy = pY;
			int xx = pX;
			
			if (move == 1) yy--;
			else if (move == 2) yy++;
			else if (move == 3) xx--;
			else if (move == 4) xx++;
			// 예외처리
			if (xx >= SIZE || xx < 0) continue;
			if (yy >= SIZE || yy < 0) continue;
			if (map[yy][xx] == WALL || map[yy][xx] == GOAL) continue;
			
			// 공을 만난경우
			if (map[yy][xx] == BALL) {
				int yyy = ballY;
				int xxx = ballX;
				
				if (move == 1) yyy--;
				else if (move == 2) yyy++;
				else if (move == 3) xxx--;
				else if (move == 4) xxx++;
				
				// 예외처리
				if (xxx >= SIZE || xxx < 0) continue;
				if (yyy >= SIZE || yyy < 0) continue;
				if (map[yyy][xxx] == WALL) continue;
				
				// 공이동
				map[ballY][ballX] = 0;
				ballY = yyy;
				ballX = xxx;
				map[ballY][ballX] = BALL;				
			}
			
			
			// 캐릭터 이동시키기
			map[pY][pX] = 0;
			pY = yy;
			pX = xx;
			map[pY][pX] = PLAYER;
			
		}
		
		
		scan.close();
		
	}

}
